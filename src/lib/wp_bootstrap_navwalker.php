...
if (!class_exists('WP_Bootstrap_Navwalker')) {
	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu
	{
		...
		public function start_lvl(&$output, $depth = 0, $args = array())
		{
			...
			$class_names = $class_names ? ' class="bg-dark-blue ' . esc_attr($class_names) . '"' : '';
		...
		public function start_el(&$output, $item, $depth = 2, $args = array(), $id = 0)
		{
			 ...
			 * icon classes form the icon markup with an <em> element. This is
			 * output inside of the item before the $title (the link text).
			 */
			$icon_html = '';
			if (!empty($icon_class_string)) {
				// Append an <em> with the icon classes to what is output before links.
				$icon_html = '<em class="' . esc_attr($icon_class_string) . '" aria-hidden="true"></em> ';
			}
		...
		private function separate_linkmods_and_icons_from_classes($classes, &$linkmod_classes, &$icon_classes, $depth)
		{
			...
			foreach ($classes as $key => $class) {
				 ...
				 */
				if (preg_match('/^disabled|^visually-hidden/i', $class) || preg_match('/^dropdown-header|^dropdown-divider|^dropdown-item-text/i', $class) && $depth > 0) {
